    ...
    static async getExecutableManifest(git: Git) {
        ...
        for (var i in executableManifest.slurm_input_rules) {
            ...
            if (!slurm_configs.includes(i)) {
                delete executableManifest.slurm_input_rules[i]; continue
            }
            ...
            if (slurm_integer_none_unit_config.includes(i)) {
                executableManifest.slurm_input_rules[i].unit = 'None'; continue
            }

            if (slurm_integer_configs.includes(i)) {
                if (!executableManifest.slurm_input_rules[i].max) {
                    executableManifest.slurm_input_rules[i].max = executableManifest.slurm_input_rules[i].default_value * 2
                }
            ...
            if (slurm_string_option_configs.includes(i)) {
                ...
                if (!executableManifest.slurm_input_rules[i].options.includes(executableManifest.slurm_input_rules[i].default_value)) {
                    executableManifest.slurm_input_rules[i].options.push(executableManifest.slurm_input_rules[i].default_value)
                }
            }
        }
        ...
        for (var i in executableManifest.param_rules) {
            ...
            if (!executableManifest.param_rules[i].default_value) {
                delete executableManifest.param_rules[i]; continue
            }

            if (!['integer', 'string_option', 'string_input'].includes(executableManifest.param_rules[i].type)) {
                delete executableManifest.param_rules[i]; continue
            }
            ...
            if (executableManifest.param_rules[i].type == 'integer') {
                if (!executableManifest.param_rules[i].max) {
                    executableManifest.param_rules[i].max = executableManifest.param_rules[i].default_value * 2
                }
            ...
            if (executableManifest.param_rules[i].type == 'string_option') {
                ...
                if (!executableManifest.param_rules[i].options.includes(executableManifest.param_rules[i].default_value)) {
                    executableManifest.param_rules[i].options.push(executableManifest.param_rules[i].default_value)
                }
            }
        }
